...
Основной инструмент — агент в Cursor. Он писал черновики и правки по задаче; контракт, инварианты и merge оставались на мне. Автокомплит «на глаз» почти не использовал: MCP, CPM и HTTP слишком легко разъезжаются, если не держать один источник правды.

**Где ассистент ускорял работу**

- Обвязка репозитория: uv workspace, Compose (dev/prod), Alembic, GitHub Actions, nginx, Bruno после изменений API.
- Черновик цикла OpenRouter → tool-calling → MCP и серверной сессии чата (cookie, TTL, SSE).
- Регрессионные тесты по конкретным сбоям агента: фокус `last_task_id`, местоимения в `reassign_tasks`, область `shift_tasks`, отказ создавать дубликат имени без явного согласия.

**Что не принимал без проверки**

- Расчёт CPM и разбор Excel (циклы, предшественники по имени или номеру строки) — только вместе с тестами `plan-core`.
- Семантика тулов. `add_predecessors` не равен `set_predecessors`; «подвинь её на N дней» не значит сдвинуть весь план. Промпт это путает, пока контракт и тесты не зафиксированы.
- Периметр: CORS allowlist, HttpOnly-cookie сессии, MCP только во внутренней сети Compose, ключ OpenRouter только в `.env` на хосте, не в секретах CI.

**Как это связано с устройством продукта**

Тот же принцип, что в рантайме: модель не ходит в Postgres. Снимок плана кладётся в каждый ход чата, id задач модель не выдумывает. Список тулов у LLM уже, чем у MCP: `delete_task` на сервере есть, в чат не отдаётся.

Промпт и allowlist не заменяют auth, аудит tool-calls и защиту от prompt injection — это сознательно оставлено в [roadmap](docs/ROADMAP_TO_PRODUCTION.md).
